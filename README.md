...
Food filters now explicitly say **Foods to EXCLUDE — leave unchecked if you eat them**. Meals shows the number of compatible recipes and the reasons for exclusions when none match. Common allergy terms are recognized, including seafood/seafoods as fish and shellfish; unknown terms still pause menus. The direct food-filter editor updates upcoming unlogged meals and groceries without rerunning setup. Empty saved menus recover when compatible recipes are available.
