...
for(let i=0,n=await page.locator('.week button').count();i<n&&!await page.locator('.pager-page[data-page=today] [data-action=start]').count();i++)await page.locator('.week button').nth(i).click();await page.evaluate(()=>document.fonts.ready);
...
let box=await page.locator('#modal').boundingBox();assert.equal(box.x,0);assert.ok(box.y>=0&&box.y<=16);assert.equal(box.width,375);assert.equal(await page.locator('#modal [data-action=close]').count(),1);
assert.ok(!(await page.locator('#modal').innerText()).includes('Choose a weight'));
const demo=await page.locator('#modal .watch-demo').boundingBox(),tech=await page.locator('#modal label').filter({hasText:'Technique'}).boundingBox();assert.ok(tech.y-demo.y-demo.height>=20);
...
const fonts=await page.evaluate(async()=>{await document.fonts.load('600 24px "Archivo"');return [document.fonts.check('600 24px "Archivo"')];});assert.ok(fonts.every(Boolean));
