...
const controlledDrag=await sheet.evaluate(node=>{const touch=y=>new Touch({identifier:2,target:node,clientX:188,clientY:y,pageX:188,pageY:y,screenX:188,screenY:y});node.dispatchEvent(new TouchEvent('touchstart',{touches:[touch(680)],changedTouches:[touch(680)],bubbles:true}));node.dispatchEvent(new TouchEvent('touchmove',{touches:[touch(800)],changedTouches:[touch(800)],bubbles:true,cancelable:true}));const far=node.style.getPropertyValue('--sheet-drag');node.dispatchEvent(new TouchEvent('touchmove',{touches:[touch(740)],changedTouches:[touch(740)],bubbles:true,cancelable:true}));const returned=node.style.getPropertyValue('--sheet-drag');node.dispatchEvent(new TouchEvent('touchend',{touches:[],changedTouches:[touch(740)],bubbles:true}));return {far,returned};});
assert.deepEqual(controlledDrag,{far:'120px',returned:'60px'});await page.waitForTimeout(300);assert.ok(await sheet.isVisible());
await drag('#modal',[188,700],[188,760],220);await page.waitForTimeout(260);assert.ok(await sheet.isVisible());
await drag('#modal',[188,700],[188,620],180);await page.waitForTimeout(260);assert.ok(await sheet.isVisible());
await drag('#modal',[188,680],[188,800],220);await page.waitForTimeout(280);assert.equal(await sheet.getAttribute('open'),null);
await page.locator('.pager-page[data-page=progress] [data-action=tdee]').click();
await sheet.evaluate(node=>node.scrollTop=node.scrollHeight);
await drag('#modal',[188,680],[188,800],220);await page.waitForTimeout(280);assert.equal(await sheet.getAttribute('open'),null);
