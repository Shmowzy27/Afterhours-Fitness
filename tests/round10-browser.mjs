...
const controlledDrag=await pull.evaluate(node=>{const touch=y=>({identifier:2,target:node,clientX:188,clientY:y,pageX:188,pageY:y,screenX:188,screenY:y});const fire=(type,touches,changedTouches,cancelable=false)=>{const event=new Event(type,{bubbles:true,cancelable});Object.defineProperties(event,{touches:{value:touches},changedTouches:{value:changedTouches}});node.dispatchEvent(event);};fire('touchstart',[touch(620)],[touch(620)]);fire('touchmove',[touch(740)],[touch(740)],true);const far=node.closest('dialog').style.getPropertyValue('--sheet-drag');fire('touchmove',[touch(680)],[touch(680)],true);const returned=node.closest('dialog').style.getPropertyValue('--sheet-drag');fire('touchend',[],[touch(680)]);return {far,returned};});
assert.deepEqual(controlledDrag,{far:'120px',returned:'60px'});await page.waitForTimeout(300);assert.ok(await sheet.isVisible());
await drag('#modal .sheet-pull-zone',[188,620],[188,680],220);await page.waitForTimeout(300);assert.ok(await sheet.isVisible());
await drag('#modal .sheet-pull-zone',[188,700],[188,620],180);await page.waitForTimeout(300);assert.ok(await sheet.isVisible());
await drag('#modal',[188,620],[188,760],220);await page.waitForTimeout(300);assert.ok(await sheet.isVisible());
await drag('#modal .sheet-pull-zone',[188,620],[188,760],220);await page.waitForTimeout(280);assert.equal(await sheet.getAttribute('open'),null);
await page.locator('.pager-page[data-page=progress] [data-action=tdee]').click();
await sheet.evaluate(node=>node.scrollTop=node.scrollHeight);
await drag('#modal .sheet-pull-zone',[188,620],[188,760],220);await page.waitForTimeout(280);assert.equal(await sheet.getAttribute('open'),null);
