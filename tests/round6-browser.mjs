...
const desktop=await page.evaluate(()=>{
 ...
 const screen=getComputedStyle(document.querySelector('.today-screen'));
 ...
 return {gaps:week.slice(1).map((box,index)=>Math.round(box.left-week[index].right)),columns:screen.gridTemplateColumns,workoutWidth:workout.width,workoutHeight:workout.height,actionWidth:action.width,gutterDelta:Math.round(Math.abs(header.left-heading.left))};
});
assert.ok(desktop.gaps.every(gap=>gap===8));
assert.match(desktop.columns,/px .*px/);
assert.ok(desktop.actionWidth<desktop.workoutWidth/2);
assert.ok(desktop.workoutHeight<=260);
